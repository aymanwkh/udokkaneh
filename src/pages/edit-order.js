...
const EditOrder = props => {
  ...
  const order = useRef(state.orders.find(o => o.id === props.id))
  const [withDelivery, setWithDelivery] = useState(order.current.withDelivery)
  const [urgent, setUrgent] = useState(order.current.urgent)
  ...
  , [orderBasket])
  useEffect(() => {
    dispatch({type: 'LOAD_ORDER_BASKET', order: order.current})
  }, [dispatch, order])
  useEffect(() => {
    if (error) {
      showError(error)
  ...
  const handleSubmit = async () => {
    try{
      setInprocess(true)
      await editOrder({...order.current, withDelivery, urgent}, state.orderBasket, state.customerInfo, state.locations)
      setInprocess(false)
      showMessage(order.current.status === 'n' ? labels.editSuccess : labels.sendSuccess)
